...
//main.addEventListener('click', chooseColor);
...
function createGrid(choice) {
    if (userChoice != null) {
        ...
        grid.setAttribute('style', `grid-template-columns: repeat(${userChoice},1fr); grid-template-rows: repeat(${userChoice},1fr);`)
...
function changeColorRainbow(e) {
    if (e.target.classList.contains('box')) {
        e.target.setAttribute('style', `background-color: rgb(${randomNum()},${randomNum()},${randomNum()})`)
    }
}
